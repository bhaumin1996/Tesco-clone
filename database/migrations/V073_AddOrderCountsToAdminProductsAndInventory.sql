-- V073_AddOrderCountsToAdminProductsAndInventory.sql
-- Author: Tesco Clone
-- Date: 2026-05-16
-- Description: Rebuild proc_Admin_GetProducts (adds @SortBy/@SortDirection, PlacedAndConfirmedCount,
--              PendingOrderCount) and proc_Admin_GetInventory (adds PlacedAndConfirmedCount,
--              PendingOrderCount). Order counts are derived via subquery on t.tblOrderLine /
--              t.tblOrder. Placed=1, Confirmed=2.
-- Dependencies: V034, V057
...
BEGIN TRY
    BEGIN TRANSACTION;
    IF NOT EXISTS (SELECT 1 FROM t.tblMigration WHERE Version = 'V073')
        INSERT INTO t.tblMigration (Version, Description)
        VALUES ('V073', 'AddOrderCountsToAdminProductsAndInventory');
    COMMIT TRANSACTION;
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
