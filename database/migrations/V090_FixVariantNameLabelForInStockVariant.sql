-- V090_FixVariantNameLabelForInStockVariant.sql
-- Author: Tesco Clone
-- Date: 2026-05-18
-- Description: Fix the UnitPrice (variant name label) subquery in
--              proc_Catalogue_GetProductById and proc_Catalogue_SearchProducts
--              to prefer the name of the lowest-ID IN-STOCK variant,
--              falling back to the lowest-ID variant if none are in stock.
--              This ensures the displayed label (e.g. "2 Litre") matches
--              the DefaultVariantId that will actually be added to the cart.
-- Dependencies: V088, V089
...
BEGIN
    ...
    BEGIN TRY
        SELECT
            ...
            p.AverageRating,
            p.ReviewCount,
            -- DefaultVariantId: prefer lowest-ID in-stock variant, fallback to any
            COALESCE(
                (SELECT MIN(dv.Id) FROM m.tblProductVariant dv
                 WHERE  dv.ProductId      = p.Id
                   AND  dv.StockQuantity  > 0
                   AND  dv.IsDeleted      = 0
            ...
            -- UnitPrice = VariantName of the SAME preferred variant (in-stock first)
            COALESCE(
...
BEGIN TRY
    BEGIN TRANSACTION;
    IF NOT EXISTS (SELECT 1 FROM t.tblMigration WHERE Version = 'V090')
        INSERT INTO t.tblMigration (Version, Description)
        VALUES ('V090', 'FixVariantNameLabelForInStockVariant');
    COMMIT TRANSACTION;
END TRY
BEGIN CATCH
