...
export class AdminBrandsComponent implements OnInit {
  ...
  protected existingLogoUrl = signal<string | null>(null);
  ...
  protected filteredBrands = computed(() => {
  ...
  protected totalPages = computed(() => Math.max(1, Math.ceil(this.filteredBrands().length / this.pageSize)));
  protected pagedBrands = computed(() => {
  ...
  private _loadAll(): void {
    ...
    this._http.get<Brand[]>(`${this._base}/brands`).subscribe({
      next: b => { this.brands.set(b); this.currentPage.set(1); this.loading.set(false); },
  ...
  protected openEdit(brand: Brand): void {
    this.editId.set(brand.brandId);
    this.form.patchValue({ name: brand.name, slug: brand.slug });
  ...
  protected save(): void {
    ...
    if (file) {
      ...
      this._doSave(this.existingLogoUrl());
    }
  }
  ...
  private _uploadThenSave(file: File): void {
    ...
    this._http.post<{ path: string }>(`${environment.apiUrl}/admin/images/upload?folder=brands`, fd).subscribe({
      next: res => {
      ...
      error: () => {
        ...
        this._showMessage('Logo upload failed.', 'error');
      }
    });
  }

  private _doSave(logoUrl: string | null): void {
    this.saving.set(true);
    const { name, slug } = this.form.getRawValue();
    const body = { name, slug, logoUrl: logoUrl || null };

    const req = this.editId()
      ? this._http.put(`${this._base}/brands/${this.editId()}`, body)
      : this._http.post(`${this._base}/brands`, body);

    req.subscribe({
      next: () => {
        this.showForm.set(false);
        this._loadAll();
        this._showMessage('Brand saved successfully.', 'success');
        this.saving.set(false);
      },
  ...
  protected deleteBrand(id: number): void {
    if (confirm('Are you sure you want to delete this brand?')) {
      this._http.delete(`${this._base}/brands/${id}`).subscribe({
        next: () => { this._loadAll(); this._showMessage('Brand deleted successfully.', 'success'); },
